...
public class Constants {
	...
	// EDINT extension: structured vocabulary labels, one annotation per vocabulary:
...
"    -omitReadme: Do not generate the generic readme.md in the output folder. \n" +
...
"    --help: Shows this message and exit.\n";
	...
	/**
		...
		document += "<script src=\"" + resourcesFolderName + "/jquery.js\"></script> \n" + "<script src=\""
				...
				+ "    <script> \n" + "function loadHash() {\n"
				+ "  jQuery(\".markdown\").each(function(el){var t=jQuery(this);if(t.find(\"pre,textarea,script,table,figure\").length>0){return;}t.after(marked.parse(t.html())).remove()});\n"
		...
		if(c.isIncludeDescription()){
			if(c.getDescriptionPath(c.getCurrentLanguage())!=null && !c.getDescriptionPath(c.getCurrentLanguage()).isEmpty()){
				document += "      loads.push(_ls(\"#description\",\""+c.getDescriptionPath(c.getCurrentLanguage())+"\")); \n";
			}else {
